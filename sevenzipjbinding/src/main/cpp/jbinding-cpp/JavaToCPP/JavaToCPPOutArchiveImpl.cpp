...
/*
 * Applies all output archive properties in one ISetProperties call. Some 7-Zip
 * handlers reset their configuration at the beginning of every call, so these
 * properties must not be applied independently.
 */
JNIEXPORT void JNICALL Java_net_sf_sevenzipjbinding_impl_OutArchiveImpl_nativeApplyFeatures(
        JNIEnv * env, jobject thiz, jstring compressionMethodProperty,
        jstring compressionMethod, jint compressionLevel, jint headerEncryption,
        jint threadCount, jstring solidBlockSpec, jboolean solidBlockSpecDefined) {
    TRACE("OutArchiveImpl::nativeApplyFeatures(). ThreadID=" << PlatformGetCurrentThreadId());

    JBindingSession & jbindingSession = GetJBindingSession(env, thiz);
    JNINativeCallContext jniNativeCallContext(jbindingSession, env);
    JNIEnvInstance jniEnvInstance(jbindingSession, jniNativeCallContext, env);

    const unsigned int maxPropertyCount = 5;
    NWindows::NCOM::CPropVariant propValues[maxPropertyCount];
    CRecordVector<const wchar_t *> names;
    unsigned int propertyCount = 0;

    UString methodPropertyName;
    UString methodName;
    if (compressionMethod != NULL) {
        if (compressionMethodProperty == NULL) {
            jniNativeCallContext.reportError(E_INVALIDARG,
                    "Compression method property name is missing.");
            return;
        }
        methodPropertyName = UString(FromJChar(env, compressionMethodProperty));
        methodName = UString(FromJChar(env, compressionMethod));
        names.Add(methodPropertyName.Ptr());
        propValues[propertyCount++] = methodName;
    }

    if (compressionLevel >= 0) {
        names.Add(L"X");
        propValues[propertyCount++] = (UInt32) compressionLevel;
    }

    if (headerEncryption >= 0) {
        names.Add(L"HE");
        propValues[propertyCount++] = headerEncryption != 0;
    }

    if (threadCount >= 0) {
        names.Add(L"MT");
        propValues[propertyCount++] = threadCount == 0 ?
                NWindows::NCOM::CPropVariant(true) :
                NWindows::NCOM::CPropVariant((UInt32) threadCount);
    }

    UString solidBlockSpecification;
    if (solidBlockSpecDefined == JNI_TRUE) {
        names.Add(L"S");
        if (solidBlockSpec == NULL) {
            propValues[propertyCount++] = false;
        } else {
            solidBlockSpecification = UString(FromJChar(env, solidBlockSpec));
            propValues[propertyCount++] = solidBlockSpecification;
        }
    }
    ...
    result = setProperties->SetProperties(&names.Front(), propValues, propertyCount);
    if (result != S_OK) {
        TRACE("Error applying archive properties. Result: 0x" << std::hex << result)
        jniNativeCallContext.reportError(result, "Error applying archive properties.");
    }
}
